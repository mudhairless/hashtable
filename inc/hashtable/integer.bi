...
type HashTableEntry_integer
    as string key
    as integer ptr value
    as ulong hash

    declare operator let ( byref x as HashTableEntry_integer )

end type

''Type: HashTableIterator(type)
''This is a prototype for a subroutine for iterating through a hashtable.
type HashTableIterator_integer as sub ( byref key as const string, byval value as integer ptr )

'' Class: HashTable_T
'' Simple to use but powerful HashTable overloaded for all built-in numerical types.
''
type HashTable_integer
public:
    ...
    ''
    declare sub Insert ( byref key_ as string, byref value as integer )
    ...
    ''Operator: []
    ''Alias for Find(key)
    declare operator [] ( byref key_ as string ) as integer ptr
    ...
    ''
    declare function Find ( byref key_ as string ) as integer ptr
    ...
    ''
    declare function Find ( byval value as const integer ptr ) as string
    ...
    ''
    declare sub ForEach ( byval iter as HashTableIterator_integer )
    ...
    declare destructor ( )
    declare operator let ( byref x as HashTable_integer )
'private:
    declare sub m_Expand ( )
    declare sub m_Insert ( byval thash as ulong, byval index as uinteger, byref key_ as string, byref value as integer )

    m_tableLength       as uinteger
    m_entryCount        as uinteger
    m_loadLimit         as uinteger
    m_primeIndex        as uinteger
    m_table             as HashTableEntry_integer ptr
