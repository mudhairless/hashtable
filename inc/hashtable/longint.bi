...
type HashTableEntry_longint
    as string key
    as longint ptr value
    as ulong hash

    declare operator let ( byref x as HashTableEntry_longint )

end type

''Type: HashTableIterator(type)
''This is a prototype for a subroutine for iterating through a hashtable.
type HashTableIterator_longint as sub ( byref key as const string, byval value as longint ptr )

'' Class: HashTable_T
'' Simple to use but powerful HashTable overloaded for all built-in numerical types.
''
type HashTable_longint
public:
    ...
    ''
    declare sub Insert ( byref key_ as string, byref value as longint )
    ...
    ''Operator: []
    ''Alias for Find(key)
    declare operator [] ( byref key_ as string ) as longint ptr
    ...
    ''
    declare function Find ( byref key_ as string ) as longint ptr
    ...
    ''
    declare function Find ( byval value as const longint ptr ) as string
    ...
    ''
    declare sub ForEach ( byval iter as HashTableIterator_longint )
    ...
    declare destructor ( )
    declare operator let ( byref x as HashTable_longint )
'private:
    declare sub m_Expand ( )
    declare sub m_Insert ( byval thash as ulong, byval index as uinteger, byref key_ as string, byref value as longint )

    m_tableLength       as uinteger
    m_entryCount        as uinteger
    m_loadLimit         as uinteger
    m_primeIndex        as uinteger
    m_table             as HashTableEntry_longint ptr
