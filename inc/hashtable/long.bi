...
type HashTableEntry_long
    as string key
    as long ptr value
    as ulong hash

    declare operator let ( byref x as HashTableEntry_long )

end type

''Type: HashTableIterator(type)
''This is a prototype for a subroutine for iterating through a hashtable.
type HashTableIterator_long as sub ( byref key as const string, byval value as long ptr )
...
public:
    ...
    ''
    declare sub Insert ( byref key_ as string, byref value as long )
    ...
    ''Operator: []
    ''Alias for Find(key)
    declare operator [] ( byref key_ as string ) as long ptr
    ...
    ''
    declare function Find ( byref key_ as string ) as long ptr
    ...
    ''
    declare function Find ( byval value as const long ptr ) as string
    ...
    ''
    declare sub ForEach ( byval iter as HashTableIterator_long )
    ...
    declare destructor ( )
    declare operator let ( byref x as HashTable_long )
'private:
    declare sub m_Expand ( )
    declare sub m_Insert ( byval thash as ulong, byval index as uinteger, byref key_ as string, byref value as long )

    m_tableLength       as uinteger
    m_entryCount        as uinteger
    m_loadLimit         as uinteger
    m_primeIndex        as uinteger
    m_table             as HashTableEntry_long ptr
