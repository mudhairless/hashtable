...
type HashTableEntry_string
    as string key
    as string ptr value
    as ulong hash

    declare operator let ( byref x as HashTableEntry_string )

end type

''Type: HashTableIterator(type)
''This is a prototype for a subroutine for iterating through a hashtable.
type HashTableIterator_string as sub ( byref key as const string, byval value as string ptr )

'' Class: HashTable_T
'' Simple to use but powerful HashTable overloaded for all built-in numerical types.
''
type HashTable_string
public:
    ...
    ''
    declare sub Insert ( byref key_ as string, byref value as string )
    ...
    ''Operator: []
    ''Alias for Find(key)
    declare operator [] ( byref key_ as string ) as string ptr
    ...
    ''
    declare function Find ( byref key_ as string ) as string ptr
    ...
    ''
    declare function FindValue ( byval value as const string ptr ) as string
    ...
    ''
    declare sub ForEach ( byval iter as HashTableIterator_string )
    ...
    declare destructor ( )
    declare operator let ( byref x as HashTable_string )
'private:
    declare sub m_Expand ( )
    declare sub m_Insert ( byval thash as ulong, byval index as uinteger, byref key_ as string, byref value as string )

    m_tableLength       as uinteger
    m_entryCount        as uinteger
    m_loadLimit         as uinteger
    m_primeIndex        as uinteger
    m_table             as HashTableEntry_string ptr
