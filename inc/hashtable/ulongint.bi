...
type HashTableEntry_ulongint
    as string key
    as ulongint ptr value
    as ulong hash

    declare operator let ( byref x as HashTableEntry_ulongint )

end type

''Type: HashTableIterator(type)
''This is a prototype for a subroutine for iterating through a hashtable.
type HashTableIterator_ulongint as sub ( byref key as const string, byval value as ulongint ptr )

'' Class: HashTable_T
'' Simple to use but powerful HashTable overloaded for all built-in numerical types.
''
type HashTable_ulongint
public:
    ...
    ''
    declare sub Insert ( byref key_ as string, byref value as ulongint )
    ...
    ''Operator: []
    ''Alias for Find(key)
    declare operator [] ( byref key_ as string ) as ulongint ptr
    ...
    ''
    declare function Find ( byref key_ as string ) as ulongint ptr
    ...
    ''
    declare function Find ( byval value as const ulongint ptr ) as string
    ...
    ''
    declare sub ForEach ( byval iter as HashTableIterator_ulongint )
    ...
    declare destructor ( )
    declare operator let ( byref x as HashTable_ulongint )
'private:
    declare sub m_Expand ( )
    declare sub m_Insert ( byval thash as ulong, byval index as uinteger, byref key_ as string, byref value as ulongint )

    m_tableLength       as uinteger
    m_entryCount        as uinteger
    m_loadLimit         as uinteger
    m_primeIndex        as uinteger
    m_table             as HashTableEntry_ulongint ptr
