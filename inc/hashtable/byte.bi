...
type HashTableEntry_byte
    as string key
    as byte ptr value
    as ulong hash

    declare operator let ( byref x as HashTableEntry_byte )

end type

''Type: HashTableIterator(type)
''This is a prototype for a subroutine for iterating through a hashtable.
type HashTableIterator_byte as sub ( byref key as const string, byval value as byte ptr )
...
public:
    ...
    ''
    declare sub Insert ( byref key_ as string, byref value as byte )
    ...
    ''Operator: []
    ''Alias for Find(key)
    declare operator [] ( byref key_ as string ) as byte ptr
    ...
    ''
    declare function Find ( byref key_ as string ) as byte ptr
    ...
    ''
    declare function Find ( byval value as const byte ptr ) as string
    ...
    ''
    declare sub ForEach ( byval iter as HashTableIterator_byte )
    ...
    declare destructor ( )
    declare operator let ( byref x as HashTable_byte )
'private:
    declare sub m_Expand ( )
    declare sub m_Insert ( byval thash as ulong, byval index as uinteger, byref key_ as string, byref value as byte )

    m_tableLength       as uinteger
    m_entryCount        as uinteger
    m_loadLimit         as uinteger
    m_primeIndex        as uinteger
    m_table             as HashTableEntry_byte ptr
